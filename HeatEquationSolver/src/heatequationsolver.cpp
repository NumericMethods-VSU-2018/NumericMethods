...
std::map<int, double> getBoundCond(
    Coord x0, Coord y0,
    const std::vector<double>& hx,
    const std::vector<double>& hy,
    MathFunc phi)
{
    std::map<int, double> cond1;
    Coord x = x0;
    Coord y = y0;
...
HeatEquationSolver::HeatEquationSolver(const InputData& data) {
    ...
    m_boundaryConditions_1 = QMap<int, double>(cond1);
    countBorderRules(k_v.first, k_v.second, cond1);
    QVector<QVector<double>> K = stdToQtMatrix(k_v.first);
    QVector<double> V = QVector<double>::fromStdVector(k_v.second);
...
QVector<double> HeatEquationSolver::temperatures() const
{
...
QVector<double> HeatEquationSolver::initialConditions() const
{
    return m_initialConditions;
}

QMap<int, double> HeatEquationSolver::boundaryConditions_1() const
{
